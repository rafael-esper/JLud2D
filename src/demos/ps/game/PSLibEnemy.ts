...
export class PSLibEnemy {
  ...
  public static initializeOriginalEnemies(): Map<GenericEnemy, Enemy> {
    const enemies = new Map<GenericEnemy, Enemy>();
			...
			this.addEnemy(enemies, PS1Enemy.MOTA_SHOOTER, new Enemy("Enemy_Mota_Shooter").setHp(59).setAtk(41).setDef(55).setExp(14).setMst(99).setNum(4).setRun(76).setTrap(40).setItem(HasItem.DIMATE).setTalk(CanTalk.YES).setChat(CanChat.YES).setSpecial(Special.DOUBLE_ATTACK).setSound(PS1Sound.ENEMY_SHOT).setAnim("battle/enemy_new/mota_shooter.chr").setMental(Mental.HIGHER).setVertical(120).setContact(130).setType(EnemyType.MOTAVIA));
